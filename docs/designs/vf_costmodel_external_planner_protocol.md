...
InsertTemplateAttributes
  ...
  - candidates[0] 是后续 ExpandTileOp 将使用的模板
  -> PreFusionAnalysis
  -> FusionPlan
...
| `candidates` | PTOAS 已排序的 TileLib 模板候选；adaptor 读取 `candidates[0]`。 |
...
| 已选模板 | `candidates[0].name` 和 `candidates[0].loop_depth` |
...
| `candidates[0].loop_depth` | CanonicalVfInfo 循环结构 | 搜索 trip count |
|---|---|---|
