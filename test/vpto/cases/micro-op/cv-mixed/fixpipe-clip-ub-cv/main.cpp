...
void LaunchFixpipe_clip_ub_cv_kernel(__fp16 *src, __fp16 *id, __fp16 *outUbClip,
                                     __fp16 *outGmClip, __fp16 *outL1Clip,
                                     void *stream);
...
int main() {
  ...
  ACL_CHECK(aclrtMalloc((void **)&outUbClipDevice, kOutSize,
                        ACL_MEM_MALLOC_HUGE_FIRST));
  ACL_CHECK(aclrtMalloc((void **)&outGmClipDevice, kOutSize,
                        ACL_MEM_MALLOC_HUGE_FIRST));
  ACL_CHECK(aclrtMalloc((void **)&outL1ClipDevice, kOutSize,
                        ACL_MEM_MALLOC_HUGE_FIRST));
  ...
  FILE_CHECK(ReadFile("./v2.bin", inputSize, srcHost, kSrcSize) && inputSize == kSrcSize,
             "./v2.bin");
  inputSize = kOutSize;
  FILE_CHECK(ReadFile("./v3.bin", inputSize, outUbClipHost, kOutSize) &&
                 inputSize == kOutSize,
             "./v3.bin");
  inputSize = kOutSize;
  FILE_CHECK(ReadFile("./v4.bin", inputSize, outGmClipHost, kOutSize) &&
                 inputSize == kOutSize,
             "./v4.bin");
  inputSize = kOutSize;
  FILE_CHECK(ReadFile("./v5.bin", inputSize, outL1ClipHost, kOutSize) &&
                 inputSize == kOutSize,
             "./v5.bin");
  ...
  LaunchFixpipe_clip_ub_cv_kernel(srcDevice, idDevice, outUbClipDevice,
                                  outGmClipDevice, outL1ClipDevice, stream);
