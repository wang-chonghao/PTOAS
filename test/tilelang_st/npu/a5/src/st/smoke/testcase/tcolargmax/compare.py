...
def main():
    ...
    for case in CASES:
        ...
        golden = np.fromfile(os.path.join(case_dir, "golden.bin"), dtype=dst_dtype).reshape(dst_shape)
        output = np.fromfile(os.path.join(case_dir, "output.bin"), dtype=dst_dtype).reshape(dst_shape)
